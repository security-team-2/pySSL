...
class ssl_server():
    ...
    def server(self, context):
        ...
        if os.path.exists(conf.CREDENTIALS): # Should exist always, since users and passwords will be predefined
            credentials = dict()
            with open(conf.CREDENTIALS,"rb") as f:
                credentials = pickle.load(f)
                f.close()

        if self.user in credentials.keys(): # We check that the user belongs to the server
    ...
    def log(self,message,display=False): # Returns by console and writes the log in the log file
